...
--as
--	begin 
--		insert into InvoiceDetail values (@InvoiceId,@TourId,@Price,@Quantity,@Paymentt,@QuantityDepartment,@Description,@PaymentMore,@Phone,@FullName,@Email);
--	end;
--go
--exec AddInvoicedetail @InvoiceId = 'tln48i0ta3ycurar0nszb8dn6a9u0n2exneoc1t8vasm2n3phqtv9poiu08bo4tv', @TourId = '4taqszx7wskp83fefjo50nqq4fgb24zsb39ygy50fa9h840dote3yrptrpi9y7yo', @Price = 1 , @Quantity = 1, @Paymentt = 1 , @QuantityDepartment = 11, @Description = N'acb', @PaymentMore = 1
--delete Invoice
--delete InvoiceDetail
--exec AddInvoice @InvoiceId = 'dssfa', @Address = 'fafdfdsaf', @MemberId = '74yugzz8wax4ko7sl1m10ltt9l5rowjgmuanxkudao9fpeyp1v5y2tefnwo6nhn0'
...
go
-- drop proc GetListCategoriesChildrent
create proc GetListCategoriesChildrent(
	@CategoryId varchar(64)
)
as
	begin
		
		if iif((select ParentId from TourCategories where TourCategoriesid = @CategoryId) is null, 0,1)  = 0
			select * from TourCategories where ParentId = @CategoryId;
		else 
